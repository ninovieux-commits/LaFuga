...
class FugaApp(App):
    ...
    def _init_firebase_token(self):
        """Initialise Firebase PAR LE CODE et récupère le token FCM.
        IMPORTANT : les classes Java sont chargées sur le THREAD PRINCIPAL. Depuis
        un thread secondaire, pyjnius utilise le classloader SYSTÈME (sans
        Firebase) → ClassNotFound. Sur le thread principal, c'est le classloader
        de l'appli (qui contient Firebase). Le travail bloquant (getToken) se fait
        ensuite dans un thread, en réutilisant les classes déjà chargées."""
        try:
            from kivy.utils import platform
            if platform != "android":
                return
        ...
        try:
            ...
            self._show_fcm_token("ERREUR (chargement classes) : " + repr(e))
            return

        def worker():
            try:
                context = classes["PythonActivity"].mActivity.getApplicationContext()
                FirebaseApp = classes["FirebaseApp"]
                inited = False
                try:
                ...
                task = classes["FirebaseMessaging"].getInstance().getToken()
                token = str(getattr(classes["Tasks"], "await")(task))
                Clock.schedule_once(
                    lambda dt, t=token: self._show_fcm_token(t), 0)
            except Exception as e:
                msg = "ERREUR Firebase : " + repr(e)
                Clock.schedule_once(
